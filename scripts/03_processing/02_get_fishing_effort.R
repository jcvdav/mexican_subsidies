...
fuel_consumption <- tracks_fixed %>% 
  ...
  mutate(vessel_rnpa = n_rnp,                                                                                                # Rename
         ...
         fuel_grams_max = (speed > 0) * 1.2 * engine_power_hp * 0.7457 * 280,
  ) %>% 
  ...
  summarize(fuel_grams = sum(fuel_grams, na.rm = T),
            fuel_grams_max = sum(fuel_grams_max)) %>%                                                                        # Calculate total daily grams
  ungroup() %>%                                                                                                              # Ungroup
  mutate(fuel_liters = (fuel_grams / 1000) / 0.91,
         fuel_liters_max = (fuel_grams_max / 1000) / 0.91)                                                                       # Convert grams to liters
